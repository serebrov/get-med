...
def get_file_name(url):
    return url.replace('https://','').replace('http://','').replace('/','_').replace(';','_').replace('?','_').replace('\n','')
...
#def save_page(soup, files, page_name=None):
    ...
    #return page_name
def convert(inp, outp, codec='utf-8',
            maxpages=0, pagenos=None,
            html=True):
    # perform conversion
...
for url in links:
    ...
    shutil.copy(f, path)
    f = open(path, 'rb')
    f_out = open(path+'_convert.html', 'wb')
    convert(f, f_out)
